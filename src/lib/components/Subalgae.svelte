<script>
    ...
    import Net from "$lib/svg/net.svg";
    import Processor from "$lib/svg/processor.svg";
    import Descore from "$lib/svg/descore.svg";
    ...
    import { color, teamKey, autoAlgaeDescoreFailure, autoAlgaeNetFailure, autoAlgaeNetSuccess, autoAlgaeProcessorSuccess, autoAlgaeProcessorFailure, autoAlgaeDescoreSuccess, teleAlgaeDescoreFailure, teleAlgaeNetFailure, teleAlgaeNetSuccess, teleAlgaeProcessorSuccess, teleAlgaeProcessorFailure, teleAlgaeDescoreSuccess} from '../../stores';
    let firstselected = 0;
    let secondselected = 0;
    export let inalgae = true;
    function success()
    {
        if (phase == "Auto")
        {
        ...
        else
        {
            ...
            {
                $teleAlgaeProcessorSuccess++;
            }
            else
            {
                $teleAlgaeDescoreSuccess++;
            }
        }
        inalgae = false;
        inalgae = inalgae;
    ...
    function falilre()
    {
        if (phase == "Auto")
        {
            ...
            {
                $autoAlgaeProcessorFailure++;
            }
            else
            {
                $autoAlgaeDescoreFailure++;
            }
        }
        else
        {
            ...
            {
                $teleAlgaeProcessorFailure++;
            }
            else
            {
                $teleAlgaeDescoreFailure++;
            }
        }
        inalgae = false; 
        inalgae = inalgae;
...
{#if firstselected == 0}
<Header phase={phase + " (Algae)"} color = {$color} team = {$teamKey}/>
<div style="height: 70vh" class="grid grid-cols-2 grid-rows-3">
<button class="grid grid-cols-1 grid-rows-2" on:click={firstselected = 1}>
    <center>
<img src={Net} alt="Net">
<h1 class="self-center">Net</h1>
</center>
</button>
<button class="grid grid-cols-1 grid-rows-2" on:click={firstselected = 2}>
<img src={Processor} alt="Processor">
<h1 class="self-center" style="font-size: 1.3rem; margin-top: 2rem">Processor</h1>
</button>
<button class="grid grid-cols-1 grid-rows-2 col-span-2" on:click={firstselected = 3}>
    <center><img src={Descore} alt="Descore">
<h1 style="margin-top: 1rem;" class="self-center">Descore</h1>
</center>
</button>
<button style="margin-left: 1rem" class="nothingg col-span-2" on:click={inalgae = false}>
...
{:else}
{#if firstselected == 1}
<Header phase={phase + " (Net)"} color = {$color} team = {$teamKey}/>
{:else if firstselected == 2}
<Header phase={phase + " (Processor)"} color = {$color} team = {$teamKey}/>
{:else}
<Header phase={phase + " (Descore)"} color = {$color} team = {$teamKey}/>
{/if}
<div class="grid grid-cols-2 grid-rows-1 justify-items-center gap-4">
    <button class="nothing" on:click={success}>
<img style="margin-left: 1rem; margin-top: 1rem" src={Success} alt="Success">
</button>
<button class="nothing" on:click={falilre}>
<img style="margin-right: 1rem; margin-top: 1rem" src={Failure} alt="">
</button>
<button class="nothing col-span-2" on:click={firstselected = 0}>
    <img style="margin-left: 1rem; margin-right: 1rem" src={Undo} alt="">
    </button>
</div>
{/if}
